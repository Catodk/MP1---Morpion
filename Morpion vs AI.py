...
def tourJ2(): #Similaire à la fonction tourJ1()
    J2 = random.choice(listeJ2)
    print("Le ... a choisi la case ",J2," !")
    for x in range (0,9):
        if choix[J2]==x:
            choix[J2]="O"
            listeJ2.remove(x)
...
def tour():
    ...
    while tours<=9: #Tant qu'il n'y a pas d'égalité
        if tours%2==0: #Le nombre de tours sera pair lorsque c'est le tour du joueur 2 et impair lorsque c'est le tour du joueur 1
            tourJ2()
            tableau()
        else:
            tourJ1()
